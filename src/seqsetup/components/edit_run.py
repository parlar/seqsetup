...
def RunStatusBar(run):
    ...
    if run.status == RunStatus.DRAFT:
        if run.validation_approved:
            actions.append(Form(
                Button(
                    Span("Mark Ready", cls="btn-text"),
                    Span("Preparing...", cls="btn-loading-text"),
                    Span(cls="spinner"),
                    type="submit",
                    cls="btn btn-primary btn-small btn-with-spinner",
                ),
        ...
        actions.append(Form(
            Button("Return to Draft", type="submit", cls="btn btn-secondary btn-small"),
...
def SampleTableSectionForRun(run, index_kits=None, test_profiles=None):
    ...
    from .wizard import SampleTableWizard
    ...
    # Only show Add Samples button when run is in DRAFT status
    add_samples_btn = None
    if is_editable:
        add_samples_btn = Div(
            A(
                "+ Add Samples",
        ...
        add_samples_btn = Div(
            P("Run is locked for editing. Return to Draft to make changes.", cls="locked-message"),
            cls="add-samples-btn",
        )

    return Div(
        add_samples_btn,
        SampleTableWizard(run, show_drop_zones=True, index_kits=index_kits, num_lanes=num_lanes, test_profiles=test_profiles, editable=is_editable),
        id="sample-section",
    )
...
def ExportPanelForRun(run):
    ...
    # Validation reports require ready status (they are generated when marking ready)
    val_json_enabled = is_ready
    val_pdf_enabled = is_ready
    ...
    buttons = [
        A(
            ...
            href=f"/runs/{run.id}/export/samplesheet-v2" if ss_enabled else None,
