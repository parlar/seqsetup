...
from ..components.wizard import IndexListCompact
...
def register(app, rt, get_index_kit_repo):
    ...
    def index_kits_page(req):
        ...
        return AppShell(
            ...
            content=IndexKitsPage(index_kit_repo.list_all(), user),
            title="Index Kits",
        )
    ...
    async def upload_index_kit(
        ...
        try:
            ...
            if index_kit_repo.exists(kit.name, kit.version):
                kits = index_kit_repo.list_all()
                return Div(
                    IndexKitSummaryTable(kits, user=user) if kits else None,
                    Div(
    ...
    def remove_index_kit(req, name: str, version: str):
        ...
        if not user.is_admin:
            kit = index_kit_repo.get_by_name_and_version(name, version)
            if not kit:
                return Div(
                    Div(
        ...
        deleted = index_kit_repo.delete(name, version)
    ...
    def get_kit_content(req, selected_kit: str = ""):
        ...
        kit = index_kit_repo.get_by_kit_id(selected_kit)
        if not kit:
            return P(f"Index kit '{selected_kit}' not found", cls="error-message")

        return IndexListCompact(kit)
