...
class SampleSheetV2Exporter:
    ...
    def export(
        ...
        # DRAGEN sections - prioritize profile-based export if repos are provided
        if test_profile_repo and app_profile_repo:
            cls._write_dragen_sections_from_profiles(
                output, run, test_profile_repo, app_profile_repo
            )
        elif cls._has_dragen_analyses(run):
            # Fallback to legacy analysis-based export
    ...
    def _write_header(cls, output: TextIO, run: SequencingRun):
        ...
        output.write(f"InstrumentPlatform,{run.instrument_platform.value}\n")
    ...
    def _write_bclconvert_settings(cls, output: TextIO, run: SequencingRun):
        ...
        # Adapter behavior
        if run.adapter_behavior:
            output.write(f"AdapterBehavior,{run.adapter_behavior}\n")
        ...
        # Other settings
        if run.create_fastq_for_index_reads:
            output.write(f"CreateFastqForIndexReads,{str(run.create_fastq_for_index_reads)}\n")

        if run.no_lane_splitting:
            output.write(f"NoLaneSplitting,{str(run.no_lane_splitting)}\n")
    ...
    def _write_bclconvert_data(cls, output: TextIO, run: SequencingRun):
        ...
        has_per_sample_mismatch1 = any(s.barcode_mismatches_index1 is not None for s in run.samples)
        has_per_sample_mismatch2 = any(s.barcode_mismatches_index2 is not None for s in run.samples)

        # Header row
        columns = []
        if has_lanes:
            columns.append("Lane")
        columns.extend(["Sample_ID", "index", "index2", "Sample_Project"])
        if has_per_sample_override:
            columns.append("OverrideCycles")
        if has_per_sample_mismatch1:
            columns.append("BarcodeMismatchesIndex1")
        if has_per_sample_mismatch2:
            columns.append("BarcodeMismatchesIndex2")
        ...
        for sample in run.samples:
            ...
            if has_per_sample_override and not override and sample.index_pair and run.run_cycles:
            ...
            for lane in lanes_to_output:
                ...
                if has_per_sample_mismatch1:
                    row.append(str(sample.barcode_mismatches_index1) if sample.barcode_mismatches_index1 is not None else "")

                if has_per_sample_mismatch2:
                    row.append(str(sample.barcode_mismatches_index2) if sample.barcode_mismatches_index2 is not None else "")
    ...
    @classmethod
    def _write_dragen_sections_from_profiles(
        cls,
        output: TextIO,
        run: SequencingRun,
        ...
        """
        Write DRAGEN sections based on ApplicationProfile definitions.

        Groups samples by test_id, resolves TestProfile -> ApplicationProfiles,
        and generates [AppName_Settings] and [AppName_Data] sections.
        """
        ...
        for test_id, samples in samples_by_test.items():
            ...
            for app_ref in test_profile.application_profiles:
                ...
                # Skip BCLConvert - it's handled separately
                if app_profile.application_name == "BCLConvert":
                    continue
