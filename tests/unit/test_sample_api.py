...
from sequencing_run_setup.models.sample_api_config import SampleApiConfig
from sequencing_run_setup.services.sample_api import check_connection, SampleApiError
...
class TestCheckConnection:
    ...
    @patch("sequencing_run_setup.services.sample_api._api_get")
    def test_successful_connection(self, mock_get):
        mock_get.return_value = [{"id": "1", "name": "WL1"}]
        config = SampleApiConfig(base_url="https://example.com/api", api_key="key")
        success, msg = check_connection(config)
        assert success is True
        assert "1 worklists" in msg
        mock_get.assert_called_once_with("https://example.com/api/worklists", "key")
    ...
    def test_non_list_response(self, mock_get):
        ...
        success, msg = check_connection(config)
        assert success is False
        assert "not a JSON array" in msg
